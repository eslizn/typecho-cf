...
export function createFlashCookieHeader(
  ...
  return `${name}=${encoded}; Path=${path}; HttpOnly; Secure; SameSite=Lax; Max-Age=${maxAge}`;
}
...
export function createFlashRedirectHeaders(location: string, name: string, value: string, path = '/'): Headers {
  const headers = new Headers();
  headers.set('Location', location);
  headers.append('Set-Cookie', createFlashCookieHeader(name, value, { path }));
  return headers;
}
