...
<div id="upload-modal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.45);z-index:1000"><div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);background:#FFF;padding:24px;border-radius:4px;width:400px;max-width:90vw"><h3 style="margin:0 0 16px;font-size:1.1em">${t('plugin.typecho-plugin-webdav.ui.uploadTitle', 'Upload files')}</h3><p style="color:#999;font-size:.92857em;margin:0 0 12px">${t('plugin.typecho-plugin-webdav.ui.uploadTo', 'Upload to:')} <span id="upload-dir-path">/</span></p><input type="file" id="upload-file-input" multiple style="margin-bottom:12px;width:100%" webkitdirectory=""><progress id="upload-progress" value="0" max="100" style="width:100%;display:none;margin-bottom:12px"></progress><div style="text-align:right"><button class="btn btn-s" id="btn-upload-cancel">${t('plugin.typecho-plugin-webdav.ui.cancel', 'Cancel')}</button><button class="btn primary btn-s" id="btn-upload-confirm">${t('plugin.typecho-plugin-webdav.ui.uploadFile', 'Upload files')}</button></div></div></div>
  <div id="mkdir-modal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.45);z-index:1000"><div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);background:#FFF;padding:24px;border-radius:4px;width:360px;max-width:90vw"><h3 style="margin:0 0 16px;font-size:1.1em">${t('plugin.typecho-plugin-webdav.ui.newFolder', 'New folder')}</h3><p style="color:#999;font-size:.92857em;margin:0 0 12px">${t('plugin.typecho-plugin-webdav.ui.createIn', 'Create under')} <span id="mkdir-dir-path">/</span></p><input type="text" id="mkdir-name-input" class="text w-100" placeholder="${t('plugin.typecho-plugin-webdav.ui.folderName', 'Folder name')}" style="margin-bottom:12px"><div style="text-align:right"><button class="btn btn-s" id="btn-mkdir-cancel">${t('plugin.typecho-plugin-webdav.ui.cancel', 'Cancel')}</button><button class="btn primary btn-s" id="btn-mkdir-confirm">${t('plugin.typecho-plugin-webdav.ui.create', 'Create')}</button></div></div></div>
  <div id="rename-modal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.45);z-index:1000"><div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);background:#FFF;padding:24px;border-radius:4px;width:360px;max-width:90vw"><h3 style="margin:0 0 16px;font-size:1.1em">${t('plugin.typecho-plugin-webdav.ui.rename', 'Rename')}</h3><input type="text" id="rename-input" class="text w-100" placeholder="${t('plugin.typecho-plugin-webdav.ui.newName', 'New name')}" style="margin-bottom:12px"><div style="text-align:right"><button class="btn btn-s" id="btn-rename-cancel">${t('plugin.typecho-plugin-webdav.ui.cancel', 'Cancel')}</button><button class="btn primary btn-s" id="btn-rename-confirm">${t('plugin.typecho-plugin-webdav.ui.confirm', 'Confirm')}</button></div></div></div>
</div>
<style>
...
function RT(){var body=document.getElementById("file-list-body");if(!entries.length){body.innerHTML='<tr><td colspan="5"><h6 class="typecho-list-table-title">'+E(M.empty||"")+'</h6></td></tr>';return}var h=entries.map(function(e){var ep=(curPath?curPath+"/":"")+e.name;var dp=e.isFolder?ep+"/":ep;var ca=e.isFolder?'href="#" data-nav="'+E(dp)+'"':'href="/api/admin/webdav?action=download&path='+encodeURIComponent(ep)+'" target="_blank"';return'<tr><td><input type="checkbox" value="'+E(dp)+'" data-is-folder="'+(e.isFolder?"1":"0")+'"></td><td><a class="file-link" '+ca+'>'+MI(e.name,e.isFolder)+E(e.name)+(e.isFolder?"/":"")+'</a></td><td>'+(e.isFolder?"-":BS(e.size))+'</td><td>'+FD(e.lastModified)+'</td><td><a href="#" class="rename-link" data-path="'+E(ep)+'" data-is-folder="'+(e.isFolder?"1":"0")+'" title="'+E(M.rename||"")+'" style="margin-right:8px"><i class="i-edit"></i></a><a href="#" class="delete-link" data-path="'+E(dp)+'" title="'+E(M.delete||"")+'"><i class="i-delete"></i></a></td></tr>'});h.push('<tr class="webdav-pager"><td colspan="5"><div style="display:flex;justify-content:center;align-items:center;gap:16px;padding:8px 0">'+(curPage>1?'<a href="#" class="pager-link" data-page="'+(curPage-1)+'">&laquo; '+E(M.previous||"")+'</a>':'<span style="color:#ccc">&laquo; '+E(M.previous||"")+'</span>')+'<span>'+E(MT("pageSummary",{page:curPage,pages:totalPages,count:totalItems}))+'</span>'+(curPage<totalPages?'<a href="#" class="pager-link" data-page="'+(curPage+1)+'">'+E(M.next||"")+' &raquo;</a>':'<span style="color:#ccc">'+E(M.next||"")+' &raquo;</span>')+'</div></td></tr>');body.innerHTML=h.join("");document.querySelectorAll(".typecho-table-select-all").forEach(function(cb){cb.checked=false})}
LD("");
})();
</script>`;
}
